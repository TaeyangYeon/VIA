...
export async function exportCode(): Promise<Blob> {
  const res = await apiClient.get('/api/export/code', { responseType: 'blob' });
  return res.data as Blob;
}

export async function exportResult(): Promise<Blob> {
  const res = await apiClient.get('/api/export/result', { responseType: 'blob' });
  return res.data as Blob;
}
