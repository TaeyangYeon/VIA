...
export default function Layout() {
  ...
  useEffect(() => {
    if (!window.via?.getEngineStatus) return;
    window.via.getEngineStatus().then((status) => {
      if (!status.connected && (!status.engine_mode || status.engine_mode === 'local')) {
        setShowOllamaGuide(true);
      }
    }).catch(() => {});
  }, []);

  const handleOllamaRetry = () => {
    if (!window.via?.getEngineStatus) return;
    window.via.getEngineStatus().then((status) => {
      if (status.connected) setShowOllamaGuide(false);
    }).catch(() => {});
  };
    ...
    <div className="flex h-screen" style={{ backgroundColor: '#0a0a0a' }}>
      {showOllamaGuide && (
        <OllamaSetupGuide
          ...
          onDismiss={() => setShowOllamaGuide(false)}
        />
      )}
      {/* Sidebar */}
      <nav
